...
class RegistrationUseCaseOrderingTest {
    ...
    private class FakeAuthApi : RegistrationAuthApiPort {
        val calls = mutableListOf<String>()
        var nextResult: AuthResult<postmark.core.data.network.RegisterResponseDto>? = null

        override suspend fun register(request: postmark.core.data.network.RegisterRequestDto): AuthResult<postmark.core.data.network.RegisterResponseDto> {
            calls += "network"
    ...
    private class FakeAccounts : CurrentAccountPort {
        val calls = mutableListOf<String>()
        var recorded: Pair<postmark.core.data.network.RegistrationUserDto, String>? = null

        override suspend fun recordCurrentAccount(user: postmark.core.data.network.RegistrationUserDto, activeKeyBundleId: String) {
            calls += "account"
            recorded = user to activeKeyBundleId
        }
    }

    private val successResponse = postmark.core.data.network.RegisterResponseDto(
    ...
    fun networkFailureLeavesNoAccountRecordButKeepsWrappedIdentity() = runTest {
        val identity = RecordingIdentityPort()
        val api = FakeAuthApi().apply { nextResult = AuthResult.Failure(postmark.core.data.network.AuthFailure.HTTP, 409) }
        val accounts = FakeAccounts()
        val useCase = RegistrationUseCase(identity, api, accounts)
