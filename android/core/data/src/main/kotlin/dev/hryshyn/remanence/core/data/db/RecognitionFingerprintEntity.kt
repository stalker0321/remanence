 ...
 * ('' only for an unattributed row, which is unreachable through owner-scoped
 * access). A persisted row is intrinsically a FRONT fingerprint; its
 * (capsule, origin) uniqueness is capsule-scoped because capsule IDs are
 * globally unique client-generated UUIDs.
 */
@Entity(
    tableName = "recognition_fingerprint",
    indices = [
        Index(value = ["capsule_id", "origin"], unique = true),
