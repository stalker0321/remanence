...
class CapsuleAcceptanceGate(
    ...
    fun verify(input: CapsuleAcceptanceInput): CapsuleAcceptanceResult {
        ...
        if (ArtifactLayoutValidator.validate(slots) !is dev.hryshyn.remanence.core.model.ArtifactLayoutValidation.Valid) {
            return CapsuleAcceptanceResult.Rejected(RejectionReason.LAYOUT_INVALID)
        }
    ...
    private fun ArtifactBinding.toSlot(): ArtifactSlot = ArtifactSlot(
        ...
        kind = when (kind) {
            dev.hryshyn.remanence.protocol.v1.ArtifactKind.RECOGNITION_MANIFEST -> CapsuleArtifactKind.RECOGNITION_MANIFEST
            dev.hryshyn.remanence.protocol.v1.ArtifactKind.CONTENT_MANIFEST -> CapsuleArtifactKind.CONTENT_MANIFEST
            dev.hryshyn.remanence.protocol.v1.ArtifactKind.PHOTO -> CapsuleArtifactKind.PHOTO
