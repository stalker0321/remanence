...
import app.postmark.memory.auth.CurrentAccountPort
import app.postmark.memory.auth.RegistrationAuthApiPort
import app.postmark.memory.auth.RegistrationIdentityPort
...
class RegistrationViewModelTest {
    ...
    fun rejectedSubmissionShowsRedactedMessageAndAllowsRetry() = runTest {
        val api = object : RegistrationAuthApiPort {
            ...
            override suspend fun register(request: RegisterRequestDto): AuthResult<RegisterResponseDto> =
                when (++requests) {
                    1 -> AuthResult.Failure(postmark.core.data.network.AuthFailure.HTTP, 409)
