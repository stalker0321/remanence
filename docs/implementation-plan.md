...
| M1-C07 | Build/encrypt one explicit front-only recognition manifest with required FRONT and fail-closed format/version parsing. | decrypt/field/version test |
...
| M1-M05 | Add weak/strong FRONT score from profile. | exact report-to-score tests |
| M1-M06 | Add FRONT design candidate ranking and bounded owner-scoped grouping. | ranking/margin tests |
| M1-M07 | Add front-only acceptance over FRONT evidence and fail closed on unsupported manifest versions. | threshold/version tests |
...
| M1-M11 | Add Scan with one required FRONT capture and no second-side state. | state/Compose tests |
...
| M1-M16 | Create preferred recipient FRONT fingerprint after verified receipt. | origin/preferred persistence test |
...
| ID | Depends on | Single outcome | Minimum verification |
| --- | --- | --- | --- |
| M2-F0-01 | P10 | Replace recognition schema/domain/index with required FRONT and design-to-many candidate identity; retain only an explicit manifest format/version parser guard. | schema, version, 0/1/N domain tests |
| M2-F0-02 | M2-F0-01 | Implement the single front-only encrypted manifest and acceptance path; preserve outer statement, envelope, AAD, hash, and owner checks. | round-trip, malformed, unsupported-version, and crypto tests |
| M2-F0-03 | M2-F0-02 | Implement clean-schema Room and local index persistence with required FRONT and offline owner-scoped candidate loading. | clean schema, account isolation, replay, and offline tests |
| M2-F0-04 | M2-F0-02,M2-F0-03 | Wire front-only outgoing encryption/staging/upload with one recognition blob and existing outer cardinality. | ciphertext/binding/cardinality tests |
| M2-F0-05 | M2-F0-02,M2-F0-03 | Wire front-only incoming acceptance and local index persistence; fail closed on malformed/unsupported recognition payloads. | incoming acceptance, account isolation, and corruption tests |
| M2-F0-06 | M2-F0-04 | Add FRONT-only Create with required FRONT and no second-side state. | Create state, manifest, cleanup, and upload tests |
| M2-F0-07 | M2-F0-05,M2-F0-06 | Add FRONT-only Scan classification for zero/one/many local candidates; one requires full E2EE verification and many never auto-opens. | no-match, single-verify, ambiguity, crypto-failure, and offline tests |

`M2-F0-07` ends the reset implementation: the matching/index seam, safe
...
This queue is intentionally not part of the current FRONT-only implementation. It
may start only after M2-F0 has evidence for owner-scoped design-to-many
classification and no-auto-open behavior.

| ID | Depends on | Single outcome | Minimum verification |
| --- | --- | --- | --- |
| M2-F1-01 | M2-F0-07 | Add a scan-scoped bounded picker for multiple plausible capsules of one FRONT design, with minimal local hints and full E2EE verification after explicit selection. | multi-match picker, no-enumeration, no-auto-open, and crypto-gate tests |
...
| M2-A11b | A11a,A09,M2-P11 | Accept one owner/capsule-scoped incoming control/index record through the existing P11 gate, including envelope HPKE open and one transport-verified front-only recognition ciphertext. | real Room/crypto owner, context, sender-trust, hash, version, and redaction matrix |
...
| M2-A12a | A11b | Define the single-version local sender index bundle plaintext/codec for one owner/capsule-bound sealed ciphertext file with crash-safe no-replace/idempotent replay semantics; no Room migration or A11d1 wiring. | deterministic codec, format limits, AAD, randomized-sealer replay, no-follow storage, durability, concurrency, cleanup, and plaintext-canary tests |
...
| M2-A16 | A15 | Persist a recipient FRONT baseline after verified single-candidate result. | generation/order test |
...
| M2-A18 | A16,A17 | Prefer recipient FRONT baseline later and retain sender FRONT fallback under documented FRONT-only weak-evidence rules. | ordering regression test |
...
| M2-A22 | A15 | Add design-to-many 0/1/many fixtures with identical FRONT designs represented as separate capsule candidates. | no-auto-open/chooser-candidate test |
