...
class ContentManifestCodecTest {
    ...
    fun decryptedPhotosRevalidateCardinality() {
        ...
        val bytes = CapsuleArtifactCryptor().decrypt(
            ...
            ArtifactAadInput(
                ...
                artifactKind = postmark.core.model.CapsuleArtifactKind.CONTENT_MANIFEST,
                ordinal = -1,
    ...
    private fun padWithExtraPhoto(manifestBytes: ByteArray): ByteArray {
        val parsed = app.postmark.protocol.v1.ContentManifest.parseFrom(manifestBytes)
        val builder = parsed.toBuilder()
        for (ordinal in 3..5) {
            builder.addPhotos(
                app.postmark.protocol.v1.PhotoEntry.newBuilder()
